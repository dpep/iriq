###  Unreleased
- **`completion fish` is now supported in both runtimes** — a shared `completions/iriq.fish` ships with the gem and is inlined in the Rust CLI. The Rust CLI previously carried its own divergent completion scripts; it now embeds byte-identical copies of the shared bash/zsh/fish scripts (parity-tested), defaults the shell from `$SHELL` like Ruby, and emits Ruby's `unknown_shell` error envelope. The `completion` subcommand is also listed in `--help` now.
- Known divergence (documented in `script/cli_parity.sh`): `--activate-above` activates a proposal under its dynamic suggested type in Ruby (`ghp`) but falls back to `opaque_id` in Rust, whose `SegmentType` is a closed enum. Scenario skipped until Rust supports dynamic synthesized types.
