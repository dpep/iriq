...
- **Corpus files iriq can't safely use are refused, not clobbered or crashed on.** A `.json` corpus that isn't a JSON object with at least one corpus key (or `{}`) exits 1 with `corpus_error` and is left untouched; a SQLite corpus written by a newer iriq (higher `schema_version`) exits 1 asking you to upgrade. JSON corpus saves use a writer-unique temp file, so concurrent saves no longer crash — but a `.json` corpus is still last-writer-wins; use `.db` for concurrent writers.
<!-- /lane: ruby -->
