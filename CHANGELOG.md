...
- **Update changed signatures.** `Corpus::open` and `save` take `impl AsRef<Path>`. `save` exports only JSON: pointed at another path with a SQLite extension, it returns `Error::Unsupported` and writes nothing (it used to write JSON under a `.db` name nothing could reopen). `normalize_identifier(&iri, hints)` and `trace_identifier(&iri, hints)` no longer take a classifier, and `Corpus::normalize_identifier` gains the same `hints` argument (`true` gives the old output). `cross_host_shapes(&corpus, n)` is now `corpus.cross_host_shapes(n)`. `activate_proposals_above` returns the proposals it activated.
...
- **Run `--reinfer` again if it overlapped other writers.** The Rust CLI's `--reinfer` (and `--activate-above`, which reinfers) against a `.db` that another process was writing could leave views that don't match the log. Likewise a long-running writer (`tail -f … | iriq -n`) kept its old recognizers when one was activated elsewhere. Once writes stop, `--reinfer` once.
...
- **`--reinfer` on SQLite is one transaction** in both runtimes, including the one `--activate-above` runs, and other writers wait for it. Ruby's failed with `corpus PATH: database is locked` while another process was writing; every Ruby corpus transaction now takes the write lock when it begins. For the Rust CLI's bug, see Fixed.
...
- **A running writer ignored recognizers another process activated,** and wrote clusters under the old types beside the new ones. Each batch (each streamed chunk, or each observation outside a batch) now picks up activations committed before it began, at the cost of one small read, and only when another process has written since.
...
- **Re-activating a recognizer piled up copies** on the live classifier and re-ran `reinfer` each time. Activation is idempotent, and a live corpus classifies exactly like the same corpus reopened.
