...
fn run(args: &[&str], stdin_data: &str) -> String {
    ...
    assert!(out.status.success(), "iriq failed: {}", String::from_utf8_lossy(&out.stderr));
...
fn cli_normalize_uses_corpus() {
    ...
    let urls: String = NAMES.iter().map(|n| format!("https://foo.com/users/{n}/profile\n")).collect();
    run(&["--corpus", cp], &urls);

    // Corpus-informed normalize: the high-cardinality literal slot collapses.
    let out = run(&["-n", "--corpus", cp, "https://foo.com/users/zoe/profile"], "");
