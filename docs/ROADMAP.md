...
3. ✅ **Calibration corpus** — 202 labeled segments across 25 types at
   ...
   and Go (`calibration_test.go`; Go port since retired) as a regression
   suite + future calibration target. Shipped in v0.13.0 (f9eb980).
...
7. ✅ **Evidence records + Trace as view** — Evidence is the structured
   ...
   render it into the same `{value, type, output, notes}` hash. Go shipped
